...
// 			} else if (session_id == null || session_id == undefined) {
// 				// 無值時需重新登入
...
(function () {
	class UserDetailApi {
		constructor() {
			...
			let session_id = sessionStorage.getItem("sessionId");
			let userType = sessionStorage.getItem("userType");
			let action = "select"; // select: 取得資料
			const urlParams = new URLSearchParams(window.location.search); // 獲取 URL 參數
			const workOrderID = urlParams.get("workOrderID"); // 檢查是否有 workOrderID

			console.log("session_id: ", session_id, "userType: ", userType, "workOrderID: ", workOrderID);

			// 優先檢查是否有 workOrderID，即使是教練身份也要顯示個案內容
			if (workOrderID) {
				$("#user-detail").show(); // 顯示個案區域
				$("#coach-detail").hide(); // 隱藏教練區域
				$("#coach-detail :input").val(""); // 清空教練欄位
				$("#personal").prop("checked", true); // 選中個人模式
				$("#personal").prop("disabled", true); // 鎖定個人 radio
				$("#coach").prop("disabled", true); // 鎖定教練 radio
				userDetailAPI(session_id, action, workOrderID); // 傳入 workOrderID 調用個案 API
			} else if (userType === "1") {
				$("#user-detail").show(); // 顯示個案區域
				$("#coach-detail").hide(); // 隱藏教練區域
				userDetailAPI(session_id, action); // 初始化時進行用戶類型的API
			} else if (userType === "2") {
				$("#coach-detail").show(); // 顯示教練區域
				$("#user-detail").hide(); // 隱藏個案區域
				coachDetailAPI(session_id, action); // 初始化時進行教練類型的API
			} else if (session_id == null || session_id == undefined) {
				// 無值時需重新登入
				window.location.assign("../LoginPage/index.html");
...
function setUserFormData(resData) {
	...
	setCheckboxData(
		...
		resData.livingEnvironmentChoice,
		resData.livingEnvironmentDescription,
	...
	setCheckboxData(
		"residentChoice",
		"residentDescription",
		resData.residentChoice,
		resData.residentDescription,
		"residentDescriptionContainer"
	); // 個案目前的同住者(複選)
	$("#mainCaregiverName").val(resData.mainCaregiverName); // 主要照顧者姓名
	setCheckboxData(
		"mainCaregiverRelationChoice",
		"mainCaregiverRelationDescription",
		resData.mainCaregiverRelationChoice,
		resData.mainCaregiverRelationDescription,
	...
	setCheckboxData(
		"phraseChoice",
		"phraseDescription",
		resData.phraseChoice,
		resData.phraseDescription,
	...
	setCheckboxData(
		"medicalSiagnosisChoice",
		"medicalSiagnosisDescription",
		resData.medicalSiagnosisChoice,
		resData.medicalSiagnosisDescription,
	...
	setCheckboxData(
		"assistiveDevicesChoice",
		"assistiveDevicesDescription",
		resData.assistiveDevicesChoice,
		resData.assistiveDevicesDescription,
		"assistiveDevicesDescriptionContainer_1"
	); // 目前正在使用的輔具(複選)
	setCheckboxData(
		"isReceivedTrainChoice",
		"isReceivedTrainDescription",
		resData.isReceivedTrainChoice,
		resData.isReceivedTrainDescription,
		"isReceivedTrainDescriptionContainer"
	); // 個案目前是否有接受復能訓練(單選)
	setCheckboxData(
		"facingDifficultyChoice",
		"facingDifficultyDescription",
		resData.facingDifficultyChoice,
		resData.facingDifficultyDescription,
